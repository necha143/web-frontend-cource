document.addEventListener('DOMContentLoaded', () => {
    ...
    позволяющее сохранять данные в формате ключ-значение,
    которые остаются доступными даже после закрытия вкладки
    или перезапуска браузера. Данные хранятся в виде строк.*/

    const form = document.getElementById('contact-me-form');
    const scheduleTable = document.getElementById('consultantions-table').querySelector('tbody');

    function loadSchedule() {
        const schedule = JSON.parse(localStoragНуe.getItem('consultationSchedule')) || [];
